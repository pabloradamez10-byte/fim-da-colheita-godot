...
func _process(delta: float) -> void:
	...
	if build_mode_0526:
		preview_refresh_0526 += delta
		if preview_refresh_0526 >= 0.07:
			preview_refresh_0526 = 0.0
...
func _load_save() -> void:
	...
	if raw_structures is Array:
		for raw_record in raw_structures as Array:
...
func _create_preview_materials_0526() -> void:
	...
	preview_valid_material_0526.roughness = 0.72
	...
	preview_invalid_material_0526.roughness = 0.72
...
func _ensure_preview_0526() -> void:
	if build_preview_0526 != null and is_instance_valid(build_preview_0526):
		return
	...
	_populate_structure_0526(build_preview_0526, selected_piece_0526, preview_valid_material_0526, false)
	build_preview_0526.rotation_degrees.y = preview_yaw_0526
...
func _update_build_preview_0526(force_material: bool = false) -> void:
	...
	var raw_target := player.global_position + facing * BUILD_DISTANCE_0526
	preview_position_0526 = Vector3(
		round(raw_target.x / BUILD_GRID_0526) * BUILD_GRID_0526,
		0.20,
		round(raw_target.z / BUILD_GRID_0526) * BUILD_GRID_0526
	)
	build_preview_0526.global_position = preview_position_0526
	build_preview_0526.rotation_degrees.y = preview_yaw_0526
	var was_valid := preview_valid_0526
	preview_valid_0526 = _can_place_structure_0526(selected_piece_0526, preview_position_0526, preview_yaw_0526)
	if force_material or was_valid != preview_valid_0526:
		_apply_preview_material_0526(preview_valid_material_0526 if preview_valid_0526 else preview_invalid_material_0526)

func _apply_preview_material_0526(material: Material) -> void:
	if build_preview_0526 == null:
		return
	var stack: Array[Node] = [build_preview_0526]
...
func _can_place_structure_0526(piece_id: String, pos: Vector3, yaw: float) -> bool:
	...
	# Pisos podem receber paredes/cercas/caixas; peças do mesmo tipo não podem ocupar a mesma célula.
	for record in structure_records_0526:
		...
		if gap < 0.55 and (piece_id == "floor" or record_type == "floor"):
			if piece_id == record_type:
				return false
			continue
		if gap < 1.05 and piece_id != "floor" and record_type != "floor":
			return false

	var shape_info := _validation_shape_0526(piece_id)
	var shape := BoxShape3D.new()
	shape.size = shape_info.get("size", Vector3.ONE) as Vector3
	var basis := Basis(Vector3.UP, deg_to_rad(yaw))
	var offset := basis * (shape_info.get("offset", Vector3.ZERO) as Vector3)
...
func place_build_preview_0526() -> bool:
	...
	var uid := "build0526:%d:%d" % [world_seed, structure_serial_0526]
	var record := {
		"uid": uid,
...
func _spawn_structure_record_0526(record: Dictionary) -> Node3D:
	...
	for raw in get_tree().get_nodes_in_group("build_structure_0526"):
	...
	if piece_id not in BUILD_PIECES_0526:
		return null
	var root := Node3D.new()
	root.name = "Built_%s_%s" % [piece_id.capitalize(), uid.replace(":", "_")]
	...
	_populate_structure_0526(root, piece_id, null, true)
	if piece_id == "crate":
...
func _populate_structure_0526(root: Node3D, piece_id: String, override_material: Material, solid: bool) -> void:
	...
	match piece_id:
		...
		"wall":
			...
			for y in [0.42, 0.82, 1.22, 1.62, 2.02]:
				_box(root, Vector3(2.62, 0.035, 0.025), Vector3(0.0, y, 0.105), dark_mat)
		...
		"crate":
			...
			_box(root, Vector3(0.08, 0.74, 0.94), Vector3(-0.48, 0.38, 0.0), dark_mat)
			_box(root, Vector3(0.08, 0.74, 0.94), Vector3(0.48, 0.38, 0.0), dark_mat)

func try_interact_near(pos: Vector3, target_player: Node) -> bool:
	var crate := _nearest_storage_crate_0526(pos)
	if crate != null:
		var uid := str(crate.get_meta("storage_uid_0526", ""))
		for raw_ui in get_tree().get_nodes_in_group("storage_ui_0526"):
			if raw_ui != null and raw_ui.has_method("open_storage_0526"):
...
func _nearest_storage_crate_0526(pos: Vector3) -> Node3D:
	...
	for raw in get_tree().get_nodes_in_group("storage_crate_0526"):
		if not (raw is Node3D):
			continue
		var crate := raw as Node3D
		if not is_instance_valid(crate):
			continue
		var distance := Vector2(pos.x - crate.global_position.x, pos.z - crate.global_position.z).length()
		if distance <= best:
			best = distance
			nearest = crate
	return nearest
...
func get_storage_total_0526(uid: String) -> int:
	var total := 0
	var storage := storage_records_0526.get(uid, {}) as Dictionary
	for raw_id in storage.keys():
...
func storage_deposit_0526(uid: String, item_id: String, amount: int, target_player: Node) -> bool:
	...
	if not bool(target_player.call("remove_item_0526", item_id, amount)):
		return false
	var storage := storage_records_0526[uid] as Dictionary
...
func new_seed() -> void:
	...
	super.new_seed()
	call_deferred("_reset_construction_root_0526")

func _reset_construction_root_0526() -> void:
	_build_construction_root_0526()
