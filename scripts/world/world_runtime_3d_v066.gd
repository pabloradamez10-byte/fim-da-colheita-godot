...
func _spawn_player() -> void:
	...
	if not state.is_empty() and player.has_method("import_save_state"):
		...
		player.global_position = _farm_to_world(Vector3(0, 0.12, 3.5))
...
func _spawn_zombies(count: int) -> void:
	...
	for i in range(spawn_count):
		...
		var spawn_pos := center + Vector3(cos(member_angle) * member_radius, 0.12, sin(member_angle) * member_radius)
...
func _restore_persistent_vehicles_0530() -> void:
	...
	for raw_uid: Variant in vehicle_records_0530.keys():
		...
		vehicle.global_position = Vector3(float(pos.get("x", 0.0)), float(pos.get("y", 0.16)), float(pos.get("z", 0.0)))

func get_hotfix_debug_066() -> Dictionary:
	var player_debug: Dictionary = {}
	if player != null and player.has_method("get_player_hotfix_debug_066"):
		player_debug = player.call("get_player_hotfix_debug_066") as Dictionary
	return {
		"version": HOTFIX_VERSION_066,
		"player": player_debug,
		"zombie_visuals": get_tree().get_nodes_in_group("zombie_visual_066").size(),
		"vehicle_visuals": get_tree().get_nodes_in_group("vehicle_art_066").size()
	}

func get_world_summary() -> Dictionary:
	var result := super.get_world_summary()
	result["hotfix_066"] = get_hotfix_debug_066()
	return result
