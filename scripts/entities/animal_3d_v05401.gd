...
func _refresh_animal_sprite_05401() -> void:
	...
	animal_sprite_05401.region_rect = Rect2(
		float(direction * ANIMAL_TILE_05401.x),
		float(row * ANIMAL_TILE_05401.y),
		float(ANIMAL_TILE_05401.x),
		float(ANIMAL_TILE_05401.y)
	)
...
func get_visual_rework_debug_05401() -> Dictionary:
	return {
		"atlas": "fdc_animal_atlas_05401.png",
