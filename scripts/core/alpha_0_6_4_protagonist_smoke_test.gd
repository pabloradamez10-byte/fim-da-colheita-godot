...
func _run() -> void:
	...
	for path in [
		"res://assets/characters/player/sprite_pack_05405/idle_8dir.png",
		"res://assets/characters/player/sprite_pack_05405/walk_8dir.png"
	]:
	...
	if not scene.has_method("get_protagonist_debug_064"):
		_fail(4, "runtime 0.6.4 do protagonista não está ativo")
		return
	var debug := scene.call("get_protagonist_debug_064") as Dictionary
	if str(debug.get("version", "")) != "0.6.4-alpha":
		_fail(5, "versão visual incorreta")
		return
	if not bool(debug.get("sprite", false)):
		_fail(6, "sprite 0.6.4 não foi instalado")
		return
	if int(debug.get("directions", 0)) != 8:
		_fail(7, "8 direções não estão ativas")
		return
	if not bool(debug.get("run_supported", false)):
		_fail(8, "corrida não está ativa")
		return

	var player := get_first_node_in_group("player") as CharacterBody3D
	if player == null:
		_fail(9, "player não foi criado")
		return
	...
	if str(debug.get("state", "")) != "walk":
		_fail(11, "caminhada não selecionada")
		return
	...
	if str(player.get_meta("player_visual_state_05405", "")) != "action":
		_fail(14, "animação de combate não permanece funcional")
		return

	print("SMOKE 0.6.4 OK | existing_protagonist=preserved 8dir idle walk run legacy_actions=preserved")
	quit(0)
