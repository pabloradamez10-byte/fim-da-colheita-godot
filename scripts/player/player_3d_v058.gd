...
func _build_visual() -> void:
	...
	# Corpo articulado: cada membro tem pivot próprio para animação procedural.
...
func _physics_process(delta: float) -> void:
	...
	var running := sprint and input_strength > 0.15 and stamina > 2.0
	if running:
	...
	# Analógico de verdade: a distância do polegar controla a velocidade.
	var base_speed := RUN_SPEED_058 if running else WALK_SPEED_058
	var analog_speed := base_speed * input_strength
	var desired := dir * analog_speed
	var accel := ACCEL_058 if input_strength > 0.02 else DECEL_058
...
func _update_character_animation(delta: float, running: bool) -> void:
	if visual_root == null or left_leg_pivot == null:
		return
	var moving := movement_amount > 0.035
	if moving:
		gait_phase += delta * lerpf(7.5, 12.5, clampf(movement_amount, 0.0, 1.0))
	var blend := clampf(movement_amount * 1.35, 0.0, 1.0)
	var leg_amp := 42.0 if running else 30.0
	var arm_amp := 25.0 if running else 18.0
	var wave := sin(gait_phase)
	var opposite := sin(gait_phase + PI)
	var target_left_leg := wave * leg_amp * blend
	var target_right_leg := opposite * leg_amp * blend
	var firearm := get_equipped_weapon() in ["pistol", "shotgun"]
	var target_left_arm := opposite * arm_amp * blend
	var target_right_arm := wave * arm_amp * blend
	if firearm:
	...
	var bob := abs(sin(gait_phase * 2.0)) * (0.055 if running else 0.038) * blend
	visual_root.position.y = lerpf(visual_root.position.y, bob, clampf(delta * 18.0, 0.0, 1.0))
	var torso_roll := sin(gait_phase) * (2.8 if running else 1.6) * blend
	...
	for raw in world.call("get_zombies"):
		if raw is Node3D:
			...
			if d < best:
				...
				nearest = zombie
	if nearest != null:
		var delta_pos := nearest.global_position - global_position
		delta_pos.y = 0.0
		if delta_pos.length() > 0.01:
			var target_yaw := atan2(delta_pos.x, delta_pos.z) + PI
...
func _animate_weapon_swing() -> void:
	...
	if right_arm_pivot == null or torso_pivot == null:
		...
		return
	var arm_start := right_arm_pivot.rotation_degrees
	var torso_start := torso_pivot.rotation_degrees
