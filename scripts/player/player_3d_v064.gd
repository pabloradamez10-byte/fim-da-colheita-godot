...
const PLAYER_IDLE_064: Texture2D = preload("res://assets/characters/player/sprite_pack_064/idle_8dir.png")
const PLAYER_WALK_064: Texture2D = preload("res://assets/characters/player/sprite_pack_064/walk_8dir.png")
const PLAYER_RUN_064: Texture2D = preload("res://assets/characters/player/sprite_pack_064/run_8dir.png")

const DIRECTION_COUNT_064 := 8
const MOVEMENT_FRAMES_064 := 4
const IDLE_FPS_064 := 2.4
const WALK_FPS_064 := 8.5
const RUN_FPS_064 := 11.5
const RUN_THRESHOLD_064 := 6.0
const SPRITE_PIXEL_SIZE_064 := 0.0155
const SPRITE_CENTER_Y_064 := 0.88
...
func _install_character_art_05402() -> void:
	...
	character_sprite_05402.hframes = MOVEMENT_FRAMES_064
	...
	set_meta("player_visual_source", "sprite_pack_064")
...
func _refresh_character_art_05402(delta: float) -> void:
	...
	# Mantém a orientação legada atualizada para as animações de combate 0.5.40.5.
	...
	if planar_speed > 0.15:
		sprite_direction_064 = _movement_direction_064(velocity)

	var next_state := "idle"
	var texture := PLAYER_IDLE_064
	var fps := IDLE_FPS_064
	if planar_speed >= RUN_THRESHOLD_064:
		next_state = "run"
		texture = PLAYER_RUN_064
		fps = RUN_FPS_064
	elif planar_speed > 0.15:
		next_state = "walk"
		texture = PLAYER_WALK_064
		fps = WALK_FPS_064
	...
	character_sprite_05402.texture = texture
	character_sprite_05402.hframes = MOVEMENT_FRAMES_064
	character_sprite_05402.vframes = DIRECTION_COUNT_064
	character_sprite_05402.pixel_size = SPRITE_PIXEL_SIZE_064
	character_sprite_05402.position.y = SPRITE_CENTER_Y_064
	character_sprite_05402.scale = Vector3.ONE
	var animation_frame := int(floor(movement_clock_064 * fps)) % MOVEMENT_FRAMES_064
	character_sprite_05402.frame = sprite_direction_064 * MOVEMENT_FRAMES_064 + animation_frame
...
func _movement_direction_064(world_velocity: Vector3) -> int:
	# Inverte a transformação isométrica usada pelo controle para obter a direção na tela.
	var screen_x := (world_velocity.x - world_velocity.z) * 0.5
	var screen_y := (world_velocity.x + world_velocity.z) * 0.5
	if absf(screen_x) < 0.001 and absf(screen_y) < 0.001:
		return sprite_direction_064
	# Ordem do atlas: baixo, baixo-esquerda, esquerda, cima-esquerda,
	# cima, cima-direita, direita, baixo-direita.
	var angle := atan2(screen_x, screen_y)
	return posmod(int(round(-angle / (PI * 0.25))), DIRECTION_COUNT_064)

func _apply_action_frame_05405() -> void:
	# As ações antigas continuam funcionais até receberem o mesmo passe visual da 0.6.4.
...
func get_player_visual_debug_064() -> Dictionary:
	return {
		...
		"source": "assets/characters/player/sprite_pack_064",
		"sprite": character_sprite_05402 != null and is_instance_valid(character_sprite_05402),
		"directions": DIRECTION_COUNT_064,
		"movement_frames": MOVEMENT_FRAMES_064,
		"idle_frames": MOVEMENT_FRAMES_064,
		"walk_frames": MOVEMENT_FRAMES_064,
		"run_frames": MOVEMENT_FRAMES_064,
