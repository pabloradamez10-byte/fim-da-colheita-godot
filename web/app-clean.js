...
const VERSION='20260720-05';
const FILES={down:`${BASE}/CHR_M_SURVIVOR_0001_S.png?v=${VERSION}`,right:`${BASE}/CHR_M_SURVIVOR_0001_E.png?v=${VERSION}`,up:`${BASE}/CHR_M_SURVIVOR_0001_N.png?v=${VERSION}`,left:`${BASE}/CHR_M_SURVIVOR_0001_W.png?v=${VERSION}`};
const colors={water:'#245d72',wetland:'#4f7055',grass:'#668b4f',soil:'#816b48',dry:'#927b58',rock:'#71746e'};
const sprites={},loaded=new Set();
for(const [dir,src] of Object.entries(FILES)){
 const im=new Image();
 im.crossOrigin='anonymous';
 im.onload=()=>{sprites[dir]=im;loaded.add(dir);if(loaded.size===4)showToast('Personagem limpo carregado.');};
 im.onerror=()=>console.error('Falha sprite',src);
 im.src=src;
}
